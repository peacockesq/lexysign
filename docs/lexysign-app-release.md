...
5. Staging mail is fail-closed on the **effective** compose server SMTP env, then on the live sink container: hostname `mailpit` or `lexysign-staging-mailpit`, port `1025`, shared app network, and no Mailpit relay/forward activation via env, config-file path, or CLI (`MP_SMTP_RELAY_CONFIG` / `MP_SMTP_FORWARD_CONFIG` / host / to / matching / true `MP_SMTP_RELAY_ALL`, plus `--smtp-relay-config`, `--smtp-forward-config`, `--smtp-relay-matching`, `--smtp-relay-all`). Every raw `Config.Env` occurrence of a supported key is inspected; an empty or inert case/duplicate variant never hides an active one. Explicitly disabled `MP_SMTP_RELAY_ALL=false` and `--smtp-relay-all=false` are not outbound. Split `--smtp-relay-all false` is treated as bare boolean activation (pflag does not consume the following token; Cobra also rejects that positional before server start, so it is not a runnable outbound witness). Mount contents are not opened or validated. Loopback UI bind and tmpfs `/tmp` database are not outbound. `.env` hostname allowlist alone is not enough. Image pin and “no actual outbound” remain separate gates.
6. Staging **and** production require a backup manifest before replacement. Archives must be distinct regular non-symlink files under `{deploy_path}/backups/`, with SHA-256 + byte-count readback, gzip streamed to EOF (CRC) for both mongo and the files tar.gz, native mongodump archive magic `6de29981` (invented `mdmp` is rejected), openable files tar without absolute/`..` members, explicit `image_swap_reverts_schema: false`, timezone-aware ISO `created_at` within 24h, and source identity bound to the target project/mongo/volume/network. Non-empty junk is rejected. This is not a restore implementation.
...
9. Post-up containers must be running with the pulled image id/revision on every inspect. Each health poll rechecks **both** app containers in the same observation pass (a previously healthy peer is not dropped). Healthchecks are required: `starting` may become `healthy` within 120s; `exited`/`dead`/wrong identity/`unhealthy` fail immediately. HTTP 200 is not a substitute. Returning from the waiter is not proof of permanent health after that pass.
...
1. Staging only: provision Mailpit as `mailpit` or `lexysign-staging-mailpit` on the staging app network, SMTP `1025`, relay/forward disabled (no config file, host/to, matching, or relay-all). Point staging `.env` at that sink. Remove Amazon SES from staging. Effective compose env must resolve to the same sink.
